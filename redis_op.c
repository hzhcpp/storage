...
/*
int set_string(redisContext *conn, char *key, char *value)
{
...
int get_string(redisContext *conn, char *key)
{
		redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int rop_string_set(redisContext *conn, const char *key, const char *value)
{
	int retn = 0;
	...
	if (reply->type != REDIS_REPLY_STATUS) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]SET %s %s error %s\n", key, value, conn->errstr);
        retn = -1;
		goto END;
	}
#endif

    LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "set %s %s succ!", key, value);
...
int rop_string_get(redisContext *conn, const char *key, char *value)
{
	...
	if (reply->type != REDIS_REPLY_STRING) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]GET %s error %s\n", key, conn->errstr);
        retn = -1;
		goto END;
	}
#endif
